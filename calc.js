...
function handleButtonClick(buttonValue) {
    ...
    function multiply(num) {
        let z = num[0];
        let y = num[1];
        let multiplyTotal = z * y;
        clearDisplay();
        updateDisplay(multiplyTotal);
    ...
    function executeOperations() {
        ...
        if (stringArrayOne.indexOf('+') > 0) {
            let str = stringArrayOne.join('');
            let arrTwo = str.split('+');
            let arrThree = [];
            for (let i = 0; i < arrTwo.length; i++) {
                arrThree.push(Number(arrTwo[i]));
            }
            add(arrThree);
        }
        if (stringArrayOne.indexOf('-') > 0) {
        ...
        if (stringArrayOne.indexOf('x') > 0) {
            let str = stringArrayOne.join('');
            let arrTwo = str.split('x');
            let arrThree = [];
            for (let i = 0; i < arrTwo.length; i++) {
                arrThree.push(Number(arrTwo[i]));
            }
    ...
    if (buttonValue === 'c') {
        stringArrayOne.length = 0;
        clearDisplay();
    } else if (buttonValue === '=') {
        executeOperations();
    } else {
        stringArrayOne.push(buttonValue);
